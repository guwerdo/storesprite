...
import { ICacheRepository } from "./interfaces/index.js";
...
@injectable()
export class UnasCacheRepository implements ICacheRepository<IProductDto> {
    private readonly validateProductDto: ValidateFunction<IProductDto>;
    ...
    constructor(
        ...
        this.validateProductDto = this._ajv.compile<IProductDto>(productDtoSchema);
    }

    public async get(key: string): Promise<IProductDto | undefined> {
        const fullKey = RedisKeys.unasCache(key);
        return await this.getByFullKey(fullKey);
    }

    public getAll(): Promise<IProductDto[] | undefined> {
        throw new Error("Not implemented");
    }

    public async add(key: string, value: IProductDto): Promise<void> {
        const fullKey = RedisKeys.unasCache(key);
    ...
    private async getByFullKey(key: string): Promise<IProductDto | undefined> {
        try {
