...
import { TEST_MODE } from "./config/config.js";
import { IAxiosHttpClient } from "./http-client/interfaces/http-client.interface.js";
import container from "./inversify.config.js";
import { ICacheRepository } from "./repository/index.js";
...
const unasCacheRepository = container.get<ICacheRepository<ProductDto>>(BindingKeys.UnasCacheRepository);
...
async function handleStreamData(row: Record<string, unknown>): Promise<void> {
    const productDto: ProductDto = {
        ...
        stockSpriteParam: unasDataSourceMapper.mapStockSpriteParam(row),
...
async function saveProduct(productDto: ProductDto): Promise<void> {
    try {
        if (!productDto.sku) {
        ...
        if (cachedProductDto) {
            const plainJsProductDto = JSON.parse(stringify(productDto)) as ProductDto;
...
async function main() {
    ...
    try {
        const unasClient = container.get<IUnasClient>(BindingKeys.IUnasClient);
        let url = TEST_MODE.UNAS.TEST_DATA_URL;
        if (!TEST_MODE.UNAS.TEST_DATA) {
            await loadWarehouses();
            url = await unasClient.getProductDb();
        }
