...
import { ICsvStreamProcessor } from "./csv-stream-processor/index.js";
...
async function main() {
    ...
    const csvStreamProcessor = container.get<ICsvStreamProcessor>(BindingKeys.ICsvStreamProcessor);
    ...
    for (const dataSource of dataSources) {
        ...
        if (data) {
            await csvStreamProcessor.process(data, dataSource.id, dataSource.productMapping);
        } else {
    ...
    await csvStreamProcessor.close();
    await redis.quit();
}
